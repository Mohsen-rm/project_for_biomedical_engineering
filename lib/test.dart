...
class _AlarmsScreenState extends State<AlarmsScreen> {
  ...
  // إعادة تعريف دالة إذن الجدولة الدقيقة
  ...
  Future<void> _initializeNotifications() async {
    ...
    const String timeZone = 'Asia/Baghdad';
    ...
    await flutterLocalNotificationsPlugin.initialize(
      ...
      onDidReceiveNotificationResponse: (NotificationResponse response) async {
        // سيتم استدعاء هذه الدالة عندما يحين وقت المنبه
        _showAlarmDetailsDialog(response.payload); // عرض التنبيه مباشرة بدون الضغط على الإشعار
      },
    );
  ...
  // تعديل دالة عرض تفاصيل المنبه لتشغيل الصوت تلقائياً
  void _showAlarmDetailsDialog(String? payload) async {
    ...
    // عرض الـ AlertDialog بعد التأكد من أن السياق موجود
    // استخدم WidgetsBinding لضمان تشغيل الكود بعد بناء الواجهة
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) {
        showDialog(
          context: context,
          barrierDismissible: false, // لمنع إغلاق الحوار بالنقر خارجها
  ...
  // تحميل المنبهات من التخزين الدائم
  ...
  // حفظ المنبهات في التخزين الدائم
  ...
  // جدولة المنبه مع عرض التنبيه تلقائياً عند الوقت المحدد
      ...
      await flutterLocalNotificationsPlugin.zonedSchedule(
        ...
        NotificationDetails(
          android: AndroidNotificationDetails(
            ...
            playSound: true,
            fullScreenIntent: true, // تفعيل الإشعار كامل الشاشة
          ),
        ),
        androidAllowWhileIdle: true,
        payload: jsonEncode(alarm.toJson()), // تمرير بيانات المنبه في الـ payload
